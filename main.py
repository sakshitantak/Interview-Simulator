...
def text_1():
    ...
    personal = {}
    personal['Extraversion'] = probas[0]
    personal['Neuroticism'] = probas[1]
    personal['Agreeableness'] = probas[2]
    personal['Conscientiousness'] = probas[3]
    personal['Openness'] = probas[4]
    
    df_text_personal = pd.DataFrame.from_dict(personal, orient='index')
    df_text_personal = df_text_personal.reset_index()
    df_text_personal.columns = ['Trait', 'Value']
    
    df_text_personal.to_csv('static/js/db/text_personal.txt', sep=',', index=False)
    ...
    with open("static/js/db/words_personal.txt", "w") as d:
        d.write("WORDS,FREQ" + '\n')
        for line in counts :
    ...
    df_words_perso = pd.read_csv('static/js/db/words_personal.txt', sep=',', error_bad_lines=False)
